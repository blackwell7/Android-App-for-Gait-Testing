...
class PoseLandmarkerHelper(
    ...
    fun detectVideoFile(
        ...
        // ADDED: Variable to collect landmark data
        val allLandmarkData = mutableListOf<String>() // NEW

        // ADDED: Indices of the landmarks of interest
        val indicesOfInterest = listOf(11, 12, 23, 24, 25, 26, 27, 28, 31, 32)  // NEW
                ...
                ?.let { frame ->
                    ...
                    poseLandmarker?.detectForVideo(mpImage, timestampMs)
                        ?.let { detectionResult ->
                            ...
                            // ADDED: Extract landmarks of interest and collect them
                            val poseLandmarks = detectionResult.poseLandmarks()          // NEW
                            if (poseLandmarks.isNotEmpty()) {                            // NEW
                                val landmarks = poseLandmarks[0].landmarkList            // NEW
                                val filteredLandmarks = indicesOfInterest.map { index -> // NEW
                                    val landmark = landmarks[index]
                                    "${landmark.x()},${landmark.y()},${landmark.z()},${landmark.visibility()}"
                                }.joinToString(",")                                      // NEW
                                val csvLine = "$timestampMs,$filteredLandmarks"          // NEW
                                allLandmarkData.add(csvLine)                             // NEW
                            }
                        } ?: run {
        ...
        val resultBundle = if (didErrorOccurred) {
            null
        } else {
            ResultBundle(resultList, inferenceTimePerFrameMs, height, width)
        }

        //ADDED: Function now returns landmark data and resultBundle
        return Pair(resultBundle, allLandmarkData)
    }
